# Ebola data
#============================================================================
ds = read.table("ebola_data1.csv",sep=" ")
# ds
...
attach(ds);plot(V1,V2, pch=20, col=c("red","blue")[unclass(as.numeric(V8)+1)], main = "Ebola Data Plot",xlab = "Ebola sentiment score", ylab = "Tweet Word Count");detach(ds)
...
mse = function(){
  ...
  for(i in 1:9){   
    ...
    logit1<-glm(V8~., sample.tr, family=binomial())
    logit1
    summary(logit1)    
    prob<<-predict(logit1, sample.te, type="response")
    length = length(sample.tr$V8)
    length_list = c(length_list, length)   
    pred_class = (matrix(c(ifelse(prob>0.5, 1, 0))))    
    table(Class=sample.te$V8, pred=pred_class)
    op_mean = mean(sample.te$V8 != pred_class)
  ...
  plot(y,x, pch=".", col="blue", main = "Logistic Regression Error \n Only Ebola Sentiment", xlab = "Training data length", ylab = "Mean Error")
